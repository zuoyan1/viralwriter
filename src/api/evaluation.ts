...
const emotionalWords: Record<string, string[]> = {
  joy: ['开心', '快乐', '幸福', '惊喜', '兴奋', '激动', '满足', '美好', '甜蜜', '温馨',
        ...
        '美丽', '漂亮', '舒服', '轻松', '愉快', '温暖', '治愈', '善意', '陪伴', '守护', '收留', '帮助', '善良'],
  ...
  sadness: ['难过', '伤心', '失望', '遗憾', '悲伤', '痛苦', '失落', '沮丧', '心碎', '绝望',
           ...
           '心情不美丽', '心情差', '天气差', '天气不好', '阴天', '下雨', '雾霾', '流浪', '孤单', '无助', '无家可归'],
...
function calculateEmotionScore(content: string): EvaluationResult['emotion'] {
  ...
  const maxScore = Math.max(...Object.values(emotionDistribution));
  
  if (maxScore === 0) {
    dominantEmotion = 'neutral';
  } else {
    dominantEmotion = Object.entries(emotionDistribution).sort((a, b) => b[1] - a[1])[0][0];
  }
